...
class BookController {
    ...
    async createBook(req, res) {
        ...
        try {
                const books = await dbService.books.create({ data: { name, author, description,  status, image: req.file.filename } })    
                res.json(books)
        } catch (error) {
    ...
    async updateBook(req, res) {
        ...
        const { id } = req.params
        try {
            const books = await bookModel.update.update(id, { name, author, description, image, price, status })
            res.json(books)
        } catch (error) {
